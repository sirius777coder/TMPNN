...
class InvariantPointAttention(nn.Module):
    ...
    def forward(
        ...
        B,L, = z.shape[:2]
        if L > self.neighbor:
            z_padding = torch.zeros((B,L,L-self.neighbor,z.shape[-1]),dtype=z.dtype,device=z.device)
            z = torch.cat([z,z_padding],dim=-2)
...
class TMPNN(nn.Module):
    ...
    def forward(self,X, S, S_mask, L, mask,device,using_ipa=True):
